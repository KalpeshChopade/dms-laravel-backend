...
function treeToHtml(tree) {
    class DisplayNode {
        ...
        toHtml() {
            ...
            <td><table>
                ...
                <tr><td colspan="2">${this.value}<\/td><\/tr>
    ...
    const getWidth = levels => levels[0].reduce((sum, {width, value}) => sum + width + (value!=null), 0);
...
function demo() {
    ...
    function randomTree(count, degree) {
        const root = new Node(randint(2000));
        let leaves = [root];
        while (count > 1) {
            const parent = choose(leaves);
            const numChildren = randint(Math.min(degree, count - 1)) + 1;
            count -= numChildren;
            parent.children = Array.from({length: numChildren}, () =>
                new Node(randint(2000))
            );
            leaves = leaves.filter(node => node != parent).concat(parent.children);
        }
        return root;
    }

    const root = randomTree(30, 6);
